...
def run_multipath_xp(nodes, test_name, setup_nets_opts, store_result_in_db_func, **kwargs):
    ...
    if setup_nets_opts.get('multipath', False):
        plugins = "-P /tmp/plugins/multipath/multipath_rtt_cond.plugin -P /tmp/plugins/multipath/addresses_filters/filter_no_v6_no_10_no_42-3.plugin"
    else:
        plugins = "-P /tmp/plugins/no_pacing/no_pacing.plugin"
    ...
    for size in file_sizes:
        ...
        results = list(filter(lambda x: x, sorted(run() for _ in range(15))))
...
if __name__ == "__main__":
    ...
    ranges = {
        ...
        "delay_ms_a_up": {"range": [10, 10], "type": float, "count": 1},  # ms
        "delay_ms_a_down": {"range": [10, 10], "type": float, "count": 1},  # ms
        "bw_b_up": {"range": [0.01, 0.01], "type": float, "count": 1},  # Mbps
        "bw_b_down": {"range": [50.0, 50.0], "type": float, "count": 1},  # Mbps
        # "loss_b": {"range": [99.9, 100], "type": float, "count": 1},  # %
        "delay_ms_b_up": {"range": [1000, 1000], "type": float, "count": 1},  # ms
        "delay_ms_b_down": {"range": [10, 10], "type": float, "count": 1},  # ms
    }
    ...
    run_experimental_design(test_nets_opts, ranges, run_multipath_xp,
                            db_filename='results_mp_asym_specific_cond.db',
